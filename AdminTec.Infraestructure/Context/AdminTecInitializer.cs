...
{
    public class AdminTecInitializer : DropCreateDatabaseAlways<AdminTecContext>
    {
        protected override void Seed(AdminTecContext context)
        {
            byte[] passToBytes = System.Text.Encoding.UTF8.GetBytes("Asdf1234");
            string pass = Convert.ToBase64String(passToBytes);

            pass = pass.Replace("+", "A");
            pass = pass.Replace(@"\", "B");
            pass = pass.Replace(@"/", "C");

            passToBytes = System.Text.Encoding.UTF8.GetBytes(pass);
            pass += Convert.ToBase64String(passToBytes);
